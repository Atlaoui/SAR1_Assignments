...
/**************************************************************/
/* determine si l'on peut emettre et identifie le destinataire dest */
inline Test_Emission (dest) {
   if
      ::(nb_voisins-1==nb_recus) -> emission = true;
		...
		do
			::(i<nb_voisins)->
				if
					::recu[voisins[i]]==0 -> dest=voisins[i];break
					::else ->i++
				fi
			::else->break
		od
...
/* Each process executes a finite loop */
   do
      ...
      ::canal_IN?type(sender,mincal) -> recu[sender]=1;wait=0;nb_recus++;
         if
            ::(minloc<mincal)->mincal=minloc;
            ::else->skip;
         fi
