...
globalFunDef returns [com.paracamplus.ilp2.interfaces.IASTfunctionDefinition node]
    : 'function' name=IDENT '(' vars+=IDENT? (',' vars+=IDENT)* ('&keys' nvars+=IDENT nvals+=expr(',' nvars+=IDENT nvals+=expr)* )?
       ')' body=expr
    ;
...
// expressions de la grammaire précédente
    ...
    | fun=expr '(' args+=expr? (',' args+=expr)* (',' ':' names+=IDENT nargs+=expr)* ')' # Invocation
    | op=('-' | '!') arg=expr # Unary
